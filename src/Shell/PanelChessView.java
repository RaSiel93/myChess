...
public class PanelChessView extends JPanel {
		...
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
				if ((x + y) % 2 == 1) {
					...
					// g2.setColor(new Color());2.setColor(new Color(30, 80,
					// 30))
	...
	private void printDangerCell(Graphics2D g2) {
		Cell cell = controller.getCellDanger();
		if (cell != null) {
	...
	private void printChessmens(Graphics g2) {
		...
		for (Chessmen chessmen : controller.getChessmens()) {
			...
			g2d.drawImage(image, chessmen.getX() * FrameMain.sizeCell,
